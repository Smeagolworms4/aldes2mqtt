...
let argError = null;
if (!args.p)  argError = 'aldes-password as required';
if (!args.l)  argError = 'aldes-login as required';
...
function publish(path: string, data: string) {
	if (client.connected) {
...
async function request(path: string, options: any = {}) {
	if (tokenIsValid(token)) {
		...
		try {
			token = await fetch(`${BASE_URL}/oauth2/token`, {
				method: 'POST',
				headers: {
...
function sendCommand(id: string, method: string, value: string) {
...
async function analyseProduct(product: any) {
	...
	if (!build) {
		...
		if (mode) {
			subscribe(`${mqttPrefix}/${details.serial_number}/set_mode`, async (value: string) => {
				try {
		...
		if (weekPlanning) {
			...
			for (const d of [ 'mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun' ]) {
				for (let h = 0; h < 24; h++) {
					subscribe(`${mqttPrefix}/${details.serial_number}/set_week_planning_${d}_${h}`, async (value: string) => {
						try {
	...
	try {
		...
		const mainLoop = async () => {
			try {
				...
				await Promise.all(
					user.products.map(analyseProduct)
				);
